...
parser.add_argument('--noise_type', type = str, default='clean')#manual
parser.add_argument('--dataset', type = str, help = 'cifar10, cifar100', default = 'cifar10')
parser.add_argument('--seed', type=int, default=1, metavar='S', help='random seed (default: 1)')
parser.add_argument('--G', type=int, default=100, help='num of rounds (parameter G in Algorithm 1)')
...
def get_2nn_acc(args, record, NumTest = 50, all_point_cnt = 15000):
    ...
    for idx in range(NumTest):
        ...
        cnt_y_3 = count_2nn_acc(KINDS, final_feat, noisy_label, all_point_cnt)
        for i in range(3):
    ...
    rec = sum([p_estimate[2][i,i,i] for i in range(KINDS)])
    print(f'2nn Acc is {rec}')
...
if __name__ == "__main__":
    ...
    get_2nn_acc(config, record, NumTest = config.G)
